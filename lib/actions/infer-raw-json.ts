...
/**
 *
 * this server actions takes raw json as an input and depending upon the content returns an array of either team objects or listing objects
 *
 * @param rawJSONContent
 * @returns
 */
export default async function InferRawJson(
  ...
  try {
    ...
    const systemPrompt = `
      ...
      Return only valid JSON in your response. No extra text or keys. If a field is absent, return null. If data is not present then return null. If data is not relevant to the schema or structur provided then return early and cancel the process.
    `;
